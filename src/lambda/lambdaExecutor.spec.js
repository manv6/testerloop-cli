...
const { handleResult, getLambdaTestResultsFromLocalBasedOnId, getFailedLambdaTestResultsFromLocal } = require("../utils/handlers");
...
describe('executeLambdas function', () => {
  ...
  it('should process lambdas and handle result when no timeouts or failures occur', async () => {
    ...
    pollLambdasWithThrottling.mockResolvedValue({ listOfLambdasWhichTimedOut: [] });
  ...
  it('should process lambdas and handle rerun when there are timeouts or failures', async () => {
    getLambdaTestResultsFromLocalBasedOnId.mockResolvedValue([ { status: "failed" }]);
    getInputData.mockResolvedValue({ rerun: true });
    pollLambdasWithThrottling.mockResolvedValueOnce({ listOfLambdasWhichTimedOut: ['lambda1'] })
